...
const Cart = ({items, unOrd, ord}) => {
  ...
  // logs go below
      ...
      // }
  console.log('ord', orderedItems)
  console.log('unOrd', unOrderedItems)
  ...
  //   })
  //   const disableItem = false //products.some(inArrHaveBeenOrdered)
  ...
  //         </div>
  //         {
  //           products.map((pr) => <CartItemProduct key= {pr.id}  product = {pr}  /> )
  //         }
  ...
  //         ? Object.keys(itemsInCart).map((store,idx) => {return <CartItem key={idx} store={store}  products={itemsInCart[store]}/> })
  //         : <NoItems/>
  //       }
  //       {/* { Object.keys(unOrderedItems).length > 0
          
  //         ? Object.keys(filtered(itemsInCart, false)).map((store,idx) => {return <CartItem key={idx} store={store}  products={itemsInCart[store]}/> })
  //         : <NoItems/>
  //       } */}
  //       {/* { Object.keys(orderedItems).length > 0
          
            
  //         ?  Object.keys(filtered(itemsInCart, true)).map((store,idx) => {return <CartItem key={idx} store={store}  products={itemsInCart[store]}/> })
  //         : null
  //       } */}
