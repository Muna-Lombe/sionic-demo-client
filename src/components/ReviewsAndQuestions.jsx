  ...
  return (
    <div className="review-questions-wrapper p-2  w-full">
      ...
      <div className="review-questions-toggle max-w-max py-2 flex  cursor-pointer  ">
        <p onClick={(e) => handleClassToggle(e, setActiveTab)} className=" PRs active py-1 px-3 border-b-2 border-blue-500 text-sm text-[arial] text-black font-medium hover:text-blue-500 transition ease-linear delay-50 ">{tags.reviewsAndQuestions.productReviewsText}</p>
        <p onClick={(e) => handleClassToggle(e, setActiveTab)} className=" QAs py-1 px-3 text-sm text-[arial] text-slate-400 font-medium border-b-[1px]  hover:text-blue-500 transition ease-in delay-100 ">{tags.reviewsAndQuestions.QAsText}</p>
      </div>
      {/* {children} */}
      {
        activeTab === "PRs"
